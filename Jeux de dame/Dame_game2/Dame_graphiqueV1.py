...
def left_clic(event): # Place un pion
    global color, oval
    X=event.x
    Y=event.y
    X -= SIZE * 0.05
...
def right_clic(event): # Place un carré / (Sélectionne le pion)
    ...
    cnv.create_rectangle(X,Y,X+ecart,Y+ecart,fill = "brown")
...
SIZE = 600  # Longueur et Hauteur du canvas (peut être modifé)

# CONSTANTES : (NE DOIT PAS ÊTRE MODIFÉE)
pad = SIZE * 0.007
pady = SIZE * 0.1
ecart = SIZE * 0.1
x = SIZE * 0.02
...
bouton_start = Button(fen, text = "Afficher le damier", command=lambda: [label_reset(alt),plateau(x,y,ecart),initPionsNoirs(SIZE,pad,pady,ecart),initPionsBlanc(SIZE,pad,pady,ecart)]
                      ,width = 20, height = 3, fg = "brown")
bouton_start.pack(side = LEFT)

bouton_player = Button(fen, text = "Fin de tour",command = lambda :[alt_joueur()],width = 20, height = 3, fg = "brown")
bouton_player.pack(side = RIGHT)

cnv.bind("<ButtonRelease-1>",left_clic)
cnv.bind("<ButtonRelease-2>",right_clic)

# Règle :
Label_regle =  Label(fen, text = "FONCTIONNEMENT : ",bg = "ivory",fg = "brown",font='Arial 10 bold').pack(
          side=TOP,expand=1)

Label_txt = Label(fen, text = "Pour déplacer un pion, click droit au centre de la pièce que vous voulez bouger,\net click gauche au centre de la case souhaitée, on peux aussi glisser déposer.",bg = "ivory",fg = "brown",font='Arial 10 bold', justify=CENTER).pack(
